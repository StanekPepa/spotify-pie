...
<template>
    <div class="bg-2ndbg rounded-lg p-6 w-9/20 aspect-square">
        <h2 class="text-2xl font-bold text-white mb-6">Top Tracks</h2>
        <div class="space-y-4 h-[calc(100%-4rem)] overflow-y-auto">
            <div v-for="(track, index) in tracks" :key="track.name" @click="openSpotifyUrl(track.spotifyUrl)"
                ...
                <span class="text-white/60 w-6 text-right">{{ index + 1 }}</span>
                <img :src="track.image" :alt="track.name" class="w-12 h-12 object-cover rounded" />
                ...
                <div class="text-white/60 font-family shrink-0">
